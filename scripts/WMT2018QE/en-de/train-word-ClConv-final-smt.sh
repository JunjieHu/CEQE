...
suffix="src\ mt\ src-mt.alignments\ tags\ pe\ ref\ src_tags\ smt.features"
...
python src/vocab.py \
    ...
    --train_feature "$baseline_feature_path/train.smt.features" \
    --include_singleton \
    --share_vocab \
    --output $vocab
fi
...
output_dir="output/WMT2018QE/word-level/$langs$mt/final/$model-embed${embed_size}-valid${valid_niter}"
mkdir -p $output_dir
